...
export default function SociosDashboard() {
    ...
    const [partnerAPct, setPartnerAPct] = useState(50);
    const [partnerBPct, setPartnerBPct] = useState(50);
    ...
    const handleAction = async (e: React.FormEvent) => {
        ...
        if (modalMode === "injection") {
            const partnerAAmount = amt * (partnerAPct / 100);
            const partnerBAmount = amt * (partnerBPct / 100);
    ...
    const partnerAWallet = getWallet("partner-a");
    const partnerBWallet = getWallet("partner-b");
                ...
                <div className="l2-border" style={{ borderRadius: 16, padding: 24, display: "flex", flexDirection: "column", gap: 16, background: "rgba(24,28,38,0.55)" }}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-base font-semibold" style={{ color: "var(--l2-fg-1)" }}>Partner A Wallet</span>
                        </div>
                    </div>
                    <div>
                        <div className="text-3xl font-bold font-mono tracking-tight" style={{ color: "var(--atlas-celestial)" }}>{formatCurrency(partnerAWallet?.balance || 0)}</div>
                ...
                <div className="l2-border" style={{ borderRadius: 16, padding: 24, display: "flex", flexDirection: "column", gap: 16, background: "rgba(24,28,38,0.55)" }}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-base font-semibold" style={{ color: "var(--l2-fg-1)" }}>Partner B Wallet</span>
                        </div>
                    </div>
                    <div>
                        <div className="text-3xl font-bold font-mono tracking-tight" style={{ color: "var(--atlas-celestial)" }}>{formatCurrency(partnerBWallet?.balance || 0)}</div>
                        <div className="text-xs mt-2 uppercase tracking-wider" style={{ color: "var(--l2-fg-3)" }}>Saldo Disponível para Saque</div>
                    </div>
                </div>
            </div>
                                    ...
                                    {splitType === "custom" && (
                                        <div className="flex items-center gap-4">
                                            <div className="flex-1">
                                                <label className="block text-[10px] uppercase mb-1" style={{ color: "var(--l2-fg-3)" }}>Partner A (%)</label>
                                                <input type="number" min="0" max="100" value={partnerAPct} onChange={(e) => { setPartnerAPct(Number(e.target.value)); setPartnerBPct(100 - Number(e.target.value)); }} className="input-l2 w-full px-3 py-1.5 rounded text-sm" />
                                            </div>
                                            <div className="flex-1">
                                                <label className="block text-[10px] uppercase mb-1" style={{ color: "var(--l2-fg-3)" }}>Partner B (%)</label>
                                                <input type="number" min="0" max="100" value={partnerBPct} onChange={(e) => { setPartnerBPct(Number(e.target.value)); setPartnerAPct(100 - Number(e.target.value)); }} className="input-l2 w-full px-3 py-1.5 rounded text-sm" />
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            {modalMode === "withdrawal" && (
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <button type="button" onClick={() => setSelectedPartner("partner-a")}
                                            className="py-3 rounded-lg border transition-all"
                                            style={{
                                                borderColor: selectedPartner === "partner-a" ? "var(--atlas-celestial)" : "var(--l2-hairline)",
                                                background: selectedPartner === "partner-a" ? "rgba(79,139,255,0.1)" : "rgba(18,21,29,0.65)",
                                                color: selectedPartner === "partner-a" ? "var(--atlas-celestial)" : "var(--l2-fg-2)"
                                            }}>Partner A</button>
                                        <button type="button" onClick={() => setSelectedPartner("partner-b")}
                                            className="py-3 rounded-lg border transition-all"
                                            style={{
                                                borderColor: selectedPartner === "partner-b" ? "var(--atlas-celestial)" : "var(--l2-hairline)",
                                                background: selectedPartner === "partner-b" ? "rgba(79,139,255,0.1)" : "rgba(18,21,29,0.65)",
                                                color: selectedPartner === "partner-b" ? "var(--atlas-celestial)" : "var(--l2-fg-2)"
                                            }}>Partner B</button>
                                    </div>
                                </div>
                            )}
