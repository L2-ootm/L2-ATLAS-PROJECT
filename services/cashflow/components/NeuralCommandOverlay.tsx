...
export default function NeuralCommandOverlay() {
    ...
    const commands = [
        ...
        { id: "add_invoice", label: "Emitir Fatura", icon: FileText, color: "text-emerald-400", action: () => { setOpen(false); setShowInvoice(true); } },
        ...
        <>
            ...
            {open && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.5)" }}>
                    ...
                    <div className="relative w-full max-w-2xl overflow-hidden flex flex-col animate-l2-enter"
                        style={{ background: "#1A1D26", border: "1px solid #2E3340", borderRadius: 12, boxShadow: "0 24px 48px rgba(0,0,0,0.4)" }}>
                        {/* Header / Input */}
                        <div className="flex items-center px-4 py-3" style={{ borderBottom: "1px solid #2E3340" }}>
                            <Search className="w-5 h-5 mr-3" style={{ color: "#5C6478" }} />
                            <input
                                autoFocus
                                value={search}
                                ...
                                style={{ color: "#F1F3F6" }}
                            />
                            <button onClick={() => setOpen(false)} className="p-1 rounded-md transition-colors" style={{ color: "#5C6478" }}
                                onMouseEnter={(e) => { e.currentTarget.style.color = "#F1F3F6"; e.currentTarget.style.background = "#252937"; }}
                        ...
                        <div className="p-2 max-h-[60vh] overflow-y-auto">
                            {filtered.length === 0 ? (
                                <p className="text-center text-sm py-8" style={{ color: "#5C6478" }}>Nenhum comando encontrado para &quot;{search}&quot;</p>
                            ) : (
                                <div className="space-y-1">
                                    <h4 className="px-3 py-2 text-[10px] uppercase tracking-wider font-bold" style={{ color: "#5C6478" }}>Ações Rápidas</h4>
                                    {filtered.map((cmd) => (
                                        <button
                                            key={cmd.id}
                                            onClick={cmd.action}
                                            ...
                                            style={{ color: "#9CA3B4" }}
                                            onMouseEnter={(e) => { e.currentTarget.style.background = "#252937"; }}
                                            onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}
                                        >
                                            <div className="p-2 rounded-lg" style={{ background: "#252937", border: "1px solid #2E3340" }}>
                                                <cmd.icon className={`w-4 h-4 ${cmd.color}`} />
                                            </div>
                                            <span className="text-sm font-medium" style={{ color: "#9CA3B4" }}>
                                                {cmd.label}
                                            </span>
                                        </button>
                                    ))}
                        ...
                        {/* Footer Hints */}
                        <div className="px-4 py-3 flex items-center justify-between" style={{ borderTop: "1px solid #2E3340", background: "#141720" }}>
                            <span className="text-[10px]" style={{ color: "#5C6478" }}>Ações rápidas</span>
                            <div className="flex items-center gap-2">
                                <kbd className="text-[10px] px-1.5 py-0.5 rounded font-mono" style={{ background: "#252937", border: "1px solid #2E3340", color: "#5C6478" }}>↑↓ navegar</kbd>
                                <kbd className="text-[10px] px-1.5 py-0.5 rounded font-mono" style={{ background: "#252937", border: "1px solid #2E3340", color: "#5C6478" }}>enter abrir</kbd>
                                <kbd className="text-[10px] px-1.5 py-0.5 rounded font-mono" style={{ background: "#252937", border: "1px solid #2E3340", color: "#5C6478" }}>esc fechar</kbd>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {!open && (
                <button
                    ...
                    title="Cmd+K"
                    onMouseEnter={(e) => { e.currentTarget.style.transform = "scale(1.08)"; e.currentTarget.style.background = "#818CF8"; }}
