...
// ATLAS ASR uses a proprietary data-URL audio format and asr_options field, not the standard OpenAI input_audio schema.
...
const VOICE_CONTROL_SYSTEM_PROMPT = `你是 ATLAS（AI 编程助手）的语音输入助手。用户通过语音向输入框口述消息，这些消息将发送给 Code Agent 执行编程任务。用户可能使用中文或英文。
