...
import { AppFileSystem } from "@atlas/shared/filesystem"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.atlas-tui`
  // folders between cwd and home, and ATLAS_TUI_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ATLAS_TUI_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ATLAS_TUI_TUI_CONFIG override, if set.
  if (Flag.ATLAS_TUI_TUI_CONFIG) {
    const configFile = Flag.ATLAS_TUI_TUI_CONFIG
  ...
  // 4. `.atlas-tui` directories (and ATLAS_TUI_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".atlas-tui") || dir === Flag.ATLAS_TUI_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".atlas-tui") && dir !== Flag.ATLAS_TUI_CONFIG_DIR) continue
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const waitForDependencies = Effect.fn("TuiConfig.waitForDependencies")((): any =>
      (Effect.forEach(deps as any, Fiber.join as any, { concurrency: "unbounded" }).pipe(Effect.ignore(), Effect.asVoid) as any),
    )
    return Service.of({ get, waitForDependencies: waitForDependencies as any })
...
async function load(text: string, configFilepath: string): Promise<Info> {
  return ConfigVariable.substitute({ text, type: "path", path: configFilepath, missing: "empty" })
    ...
    .then((data) => {
      ...
      // (mirroring the old atlas-tui.json shape) still get their settings applied.
