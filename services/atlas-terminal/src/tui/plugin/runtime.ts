...
import { Flock } from "@atlas/shared/util/flock"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".atlas-tui"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".atlas-tui", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".atlas-tui", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.ATLAS_TUI_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.ATLAS_TUI_PURE && config.plugin_origins?.length) {
        ...
        // File-based TUI plugins from .atlas-tui/tui/*.{ts,tsx}
        const configDirs = [
          path.join(Global.Path.config, "tui"),
          path.join(cwd, ".atlas-tui", "tui"),
        ]
