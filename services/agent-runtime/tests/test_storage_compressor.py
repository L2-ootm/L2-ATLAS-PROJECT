"""Tests for atlas_runtime.storage_compressor — tool-output compression (Phase 3 Track A, F6).
...
def test_terminal_adapter_passes_through_small_output():
    adapter = storage_compressor.TerminalAdapter()
    raw = "hello world"
...
def test_read_file_adapter_passes_through_small_output():
    adapter = storage_compressor.ReadFileAdapter()
    raw = "1|import os\n2|print('hi')\n"
...
def test_read_file_adapter_compresses_large_plain_text_and_preserves_path():
    ...
    assert result.original_hash == storage_compressor._hash(body)
...
def test_registry_has_default_adapters():
    assert isinstance(storage_compressor.get_adapter("terminal"), storage_compressor.TerminalAdapter)
    assert isinstance(storage_compressor.get_adapter("read_file"), storage_compressor.ReadFileAdapter)
    assert storage_compressor.get_adapter("no_such_tool") is None


def test_register_adapter_overrides():
    class _Stub:
        def compress(self, raw_output, tool_args):
            return storage_compressor.CompressedOutput(
    ...
    storage_compressor.register_adapter("stub_tool", _Stub())
    try:
        assert storage_compressor.compress_tool_output("stub_tool", {}, "anything") == "STUB"
    finally:
        del storage_compressor._REGISTRY["stub_tool"]  # test isolation — don't leak into other tests


def test_compress_tool_output_noop_for_unknown_tool():
    raw = "x" * 5000
    assert storage_compressor.compress_tool_output("web_fetch", {"url": "https://example.com"}, raw) == raw


def test_compress_tool_output_workspace_read_op_uses_read_file_adapter():
    body = "\n".join(f"{i}|x" for i in range(1, 400))
    compressed = storage_compressor.compress_tool_output("workspace", {"op": "read", "path": "a.py"}, body)
...
def test_compress_tool_output_workspace_non_read_op_is_noop():
    raw = "y" * 5000
    compressed = storage_compressor.compress_tool_output("workspace", {"op": "list", "path": "."}, raw)
...
def test_compress_tool_output_never_raises_on_adapter_exception():
    ...
    storage_compressor.register_adapter("boom_tool", _Boom())
    try:
        raw = "unchanged"
        assert storage_compressor.compress_tool_output("boom_tool", {}, raw) == raw
    finally:
        del storage_compressor._REGISTRY["boom_tool"]
