...
Source URLs:
- Reference page: https://reactbits.dev/animations/metallic-paint
- Component source: https://raw.githubusercontent.com/DavidHDev/react-bits/main/src/content/Animations/MetallicPaint/MetallicPaint.jsx
- CSS: https://raw.githubusercontent.com/DavidHDev/react-bits/main/src/content/Animations/MetallicPaint/MetallicPaint.css

## Category

Animations. Turns a (transparent-background) logo/shape image into a flowing liquid-chrome / iridescent metallic surface, with animated refraction, chromatic spread, contour shading, and an optional mouse-driven animation mode.

## Dependencies

- Raw WebGL2 (`canvas.getContext('webgl2')`) — no three/ogl. GLSL ES 3.00 (`#version 300 es`).
- A CPU image-processing pre-pass (`processImage`) on a 2D canvas that computes a signed-distance-like depth field via a Gauss-Seidel / SOR (successive over-relaxation) Laplace solve (200 iterations, omega = 1.85), packing depth in RGB and the shape alpha in A. This depth texture is what the shader reads (`u_tex`).
- `./MetallicPaint.css` (sizing/object-fit).
- Browser APIs: `Image` (with `crossOrigin='anonymous'`), `requestAnimationFrame`, `mousemove`, `devicePixelRatio`.
...
The effect has two stages. Stage 1 (CPU, `processImage`) reads the source image into a 2D canvas, classifies background vs shape pixels (near-white-opaque or near-transparent counts as background), marks the shape boundary, and then solves a Poisson/Laplace-like equation inside the shape using 200 iterations of successive over-relaxation (`omega = 1.85`) with a constant source term, producing a smooth "distance from edge" field `u`. That field is normalized and written into the texture as `gray = 255 * (1 - depth^2)` in RGB with the original alpha in A — effectively a depth/contour map of the logo. Stage 2 (GPU) renders a full-screen quad in WebGL2: the fragment shader samples that depth (`dp`) and shape mask, builds a 3D Perlin-style noise (`pW`/`aF`/`lM`) advected by `u_time` to animate iridescent flow, computes a curvature term `cv` and a fresnel rim, then derives per-channel phase values (`rP`/`gP`/`bP`) that are offset differently for R/G/B to create chromatic separation. A custom multi-band gradient function `mG` maps each phase through several `smoothstep` edges between `u_lightColor` and `u_darkColor`, giving the banded metallic sheen; contrast, tint screen-blend, and the shape mask alpha finish the pixel. Animation can be time-driven (`speed`) or mouse-driven (`mouseAnimation`, where cursor position drives the time value).

## Svelte 5 port note

Both stages port cleanly. The GLSL (`vertexShader`/`fragmentShader`) and the raw-WebGL2 plumbing in `initGL`/`uploadTexture` are framework-agnostic and reuse verbatim inside `onMount`. `processImage` is plain JS and moves unchanged; note it is CPU-heavy (200 iterations over up to 1000x1000 px) — consider running it in a Web Worker or `requestIdleCallback` to avoid blocking the cockpit's first paint, and the source image must be a transparent-background mark (PNG/SVG-rasterized) for the background classification to work. Replace `useState(ready)`/`useState(textureReady)` with `$state(false)` since the render loop gates on them; replace the imperative `useRef`s with plain `let`. Map the uniform-pushing `useEffect` to an `$effect` that reads the relevant props and calls `gl.uniform*`. Props via `let { imageSrc, tintColor = '#feb3ff', lightColor = '#ffffff', darkColor = '#000000', ... } = $props()`. For the L2 dark theme, set `darkColor` to the void background and `lightColor`/`tintColor` to a brand accent (e.g. obsidian + a cyan/violet tint) so the chrome reads against `bg-black`; container class `class="block h-full w-full object-contain"`. No gsap/framer-motion. Respect `prefers-reduced-motion` by freezing `u_time`. The image fetch uses `crossOrigin='anonymous'`, so serve the logo same-origin or with CORS headers.
