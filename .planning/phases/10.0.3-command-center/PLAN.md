...
| **WP-1** | **Async run executor** (background execution) | **CORE DONE** (`b2411d4`) | `run_executor.py` (+5 tests): `execute_run()` drives a run to terminal state (reusable by CLI `--execute`); `start_and_execute_async()` runs it on a background daemon thread, returns `run_id` immediately; owns the terminal transition, defends crashes, respects mid-flight cancel. **Pending: gateway wiring** — decision: detached CLI subprocess vs long-lived Python daemon (the gateway is Rust and dispatches the CLI; an in-process thread dies when a CLI subprocess exits, so background execution needs a detached process or a daemon hosting the thread). |
| WP-2 | `Focus` entity + `focus_service` (+ migration 0009) | **SERVICE DONE** (`ea01d4a`) | `atlas_core.schemas.Focus` + `0009_focus.sql` + `focus_service.py` (+9 tests): single-Current-Focus invariant, CRUD, JSON list fields. **Pending: gateway CRUD** (`/v1/focus`) + CLI `atlas focus` group, per the P3 Project pattern. |
