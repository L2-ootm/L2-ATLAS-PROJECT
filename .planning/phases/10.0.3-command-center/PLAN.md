...
| WP-2 | `Focus` entity + `focus_service` (+ migration 0009) | **SERVICE + CLI DONE** (`ea01d4a`,`e564c4a`) | Model + `0009_focus.sql` + `focus_service.py` + `atlas focus create/show/list/archive` (JSON). **Pending: gateway CRUD** (`/v1/focus`) per the P3 Project pattern (Rust + db.rs read fns). |
...
| WP-4 | Command Center dashboard (CC-1 UI) | TODO | `/command` React route: Current Focus, framework, priorities, quick-capture, live activity feed (reuses `useRunStream`+SSE). Needs WP-2 gateway CRUD first. |
