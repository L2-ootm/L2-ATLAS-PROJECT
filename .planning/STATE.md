...
last_activity: 2026-07-17 -- Distribution sprint complete on codex/chat-actor-workspace (5 commits): WebUI action slash commands (/help /new /clear /agent /bind /unbind /go), playbook-grade installer + @l2/atlas metadata, public README rewrite with media slots, GSD/L2 framework (skills/atlas/gsd + bundled modules/gsd with /gsd-* commands), pre-public security gate PASS (repo not flipped; report in .planning/reports/). Verification: WebUI 149, agent-runtime 922/1, atlas-cli 20/20. Owed: clean-VM installer UAT, /gsd live UAT, TUI action-command parity, media assets.
prior_activity_2026_07_17: Graph route stale-chunk recovery and TUI-style upward slash dropdown complete. Lazy Graph/Chat/Console/module chunks now perform one guarded document reload when an open page references a removed build hash, then fall through to an ATLAS recovery surface without loops. Slash completion is an unclipped sibling above the composer. Verification: WebUI 139 + TypeScript/lint/build/budget; browser Graph route and measured upward dropdown geometry green with zero console errors.
